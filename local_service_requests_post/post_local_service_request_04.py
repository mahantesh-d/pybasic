...
for x in range(4000):
    ...
    with open('local_service_requests4.json', 'r+', encoding='utf8') as f:
        data = json.load(f)
        data['OrderRef'] = a
        f.seek(0)
    ...
    r = requests.post(url, data=open('local_service_requests4.json', 'rb'), headers=headers)
    print("Time ",r.elapsed)
    print('Status Code: ',r.status_code)
    print(r.reason)
    r.close()
    with open('out11.txt', 'a') as f:
